...
var LocalizationCompiler = /** @class */ (function () {
    ...
    LocalizationCompiler.prototype.OnLocalizationDataChanged = function () {
        ...
        fs.truncate(this.addon_english_filepath, function () {
            ...
            // Go over talents
            for (var _k = 0, _l = _this.TalentArray; _k < _l.length; _k++) {
                var hero_talent_list = _l[_k];
                var talent_classname = "\t\t\"DOTA_Tooltip_" + hero_talent_list.talent_classname;
                var talent_counter = 1;
                for (var _m = 0, _o = hero_talent_list.talents; _m < _o.length; _m++) {
                    var talent = _o[_m];
                    var talent_string = talent_classname + "_" + talent_counter;
                    // Talent name
                    localization_content += talent_string + "\" \"" + talent.name + "\"";
                    localization_content += "\n";
                    // Talent description
                    var talent_description = _this.TransformForLocalization(talent.description, false);
                    localization_content += talent_string + "_Description\" \"" + talent_description + "\"";
                    localization_content += "\n";
                    // Talent lore
                    localization_content += talent_string + "_Lore\" \"" + talent.lore + "\"";
                    localization_content += "\n";
            ...
            // Go over modifiers
            for (var _p = 0, _q = _this.ModifierArray; _p < _q.length; _p++) {
                var modifier = _q[_p];
