...
export function GenerateLocalizationData(): LocalizationData {
	...
	Abilities.push({
		...
		reimagined_effects: [
...

			{
				title: "Running Nose",
				description:
					"Units afflicted with at least ${running_nose_stacks_threshold} stacks of Nasal Goo sneeze every ${running_nose_sneeze_interval} seconds, sending Nasal Goo projectiles towards their allies in ${running_nose_radius} radius of them.",
			},

			{
				title: "Raging Snot",
				description:
					"If Bristleback has at least ${raging_snot_warpath_stacks} Warpath stacks, Nasal Goo refreshes its cooldown immediately when cast. Has an internal cooldown of ${raging_snot_internal_cooldown} seconds.",
			},
		],
	...
	Abilities.push({
		...
		name: "Quill Spray",
		description:
			"Sprays enemy units with quills dealing damage in an area of effect around Bristleback. Deals bonus damage for every time a unit was hit by Quill Spray in the last ${quill_stack_duration} seconds.",
		reimagined_effects: [
			{
				title: "Needle Spreader",
				description:
					"Quill Spray can be set to auto cast. When cast with auto cast, Bristleback continually sprays quills around him, firing a wave of quills every ${needle_spreader_interval} seconds for a total of ${needle_spreader_total_instances} waves. Increases Quill Spray's mana cost to ${needle_spreader_manacost} and cooldown to ${needle_spreader_cooldown}. Bristleback is rooted and disarmed during the effect.",
			},
...

			{
				title: "Raging Quills",
				description:
					"Casting Quill Spray while Bristleback has at least 5 stacks of Warpath multiplies Quill Spray's radius by ${raging_quills_radius_multiplier}, and the projectile speed increases by ${raging_quills_projectile_speed_bonus_pct}%.",
			},
		],
		lore: "An enforcer's honor can be a prickly thing. So can his quills.",
		notes: ["Quill Spray damage is not reduced by damage block abilities.", "Spiked Edge's damage increase is still capped under the maximum damage cap."],
		ability_specials: [
			{
		...
		reimagined_effects: [
			{
				title: "Turtleback",
				description:
					"When Bristleback takes damage from the rear, the damage reduction granted by the ability increases by ${turtleback_back_dmg_reduction}% per damage instance, or by ${turtleback_side_dmg_reduction}% if the damage was taken from the side. This bonus resets when being attacked from the front, or after Bristleback doesn't take damage for ${turtleback_reset_time} seconds.",
			},

			{
				title: "Moving Fortress",
				description:
					"Can be toggled on to apply the damage reduction from the rear to the sides and grant an additional ${moving_fortress_damage_reduction_bonus}% damage reduction. However, Bristleback is slowed by ${moving_fortress_move_slow_pct}% move speed and ${moving_fortress_attack_speed_slow} attack speed while this effect is active.",
			},
		...
		reimagined_effects: [
			{
				title: "Simmer Down",
				description:
					"Warpath can be activated to consume all current Warpath stacks. Each stack consumed grants Bristleback ${simmer_down_damage_resistance_bonus_per_stack}% damage resistance from all directions for ${simmer_down_duration} seconds. While Simmer Down is active, Warpath stacks cannot be generated.",
			},

			{
				title: "Anger Burst",
				description:
					"When Bristleback accumulates over ${anger_burst_damage_threshold} damage after reductions, he generates a Warpath stack. The damage counter resets after not taking any damage over ${anger_burst_reset_time} seconds.",
			},

			{
				title: "Tantrum",
				description:
					"Warpath's stack count can keep increasing above the maximum limit. However, those additional stacks only last for ${tantrum_stack_duration} seconds and provide ${tantrum_stack_bonuses_pct}% of the bonuses.",
			},
		],
	...
	Abilities.push({
		...
		name: "Hairball",
		description:
			"Coughs a quill-packed hairball towards the target location. Erupts at the location, hitting enemies in ${radius} radius with ${nasal_goo_count} Nasal Goos and ${quill_spray_count} Quill Sprays.",
	});
		...
		talents: [
			{
				name: "Mucus Goo",
				description:
					"Each Viscous Nasal Goo projectile that hits its target increases the stack count of its Quill Spray's debuff by ${stacks}. Adds the debuff if it does not exist yet on the target.",
...

			{
				name: "Quillgun",
				description:
					"When there is only one enemy in the cast range, fires additional ${additional_quills} quills at that enemy. Those quills do not increase the debuff's stack count, but do bonus damage based on it.",
...

			{
				name: "Bristlefront",
				description:
					"While Moving Fortress is active, ${damage_taken_to_threshold_pct}% of damage taken from the front and the sides also counts toward the passive Quill Spray damage trigger.",
...

			{
				...
				description:
					"Attackers that hit Bristleback on the back have ${damage_reflection_pct}% of their own damage before reductions reflected back at them. This damage is flagged as reflection damage.",
...

			{
				...
				description:
					"Simmer Down also grants ${cooldown_reduction_per_stack}% cooldown reduction per Warpath stack consumed, up to a maximum of ${max_cooldown_reduction}% cooldown reduction.",
