...
export function GenerateLocalizationData(): LocalizationData {
		...
		reimagined_effects: [
			{
				...
				description:
					"Leaves a field on the ground where Crystal Nova was cast for ${snowstorm_duration} seconds. Allies standing on the field are granted ${snowstorm_damage_reduction}% damage reduction and ${snowstorm_status_reduction}% status resistance.",
			},
	...
	Abilities.push({
		...
		reimagined_effects: [
...

			{
				...
				description:
					"While a unit is afflicted with Frostbite, on every tick, the closest enemy in ${frost_emanation_search_radius} radius will also be afflicted with a ${frost_emanation_duration} seconds Frostbite.",
			},

			{
				title: "Igloo Frosting",
				description:
					"Can be cast on an ally, rooting and disarming it and increasing Arcane Aura's effect on that ally by ${igloo_frosting_arcane_aura_multiplier} times. Does not deals damage to the target ally.",
			},
		],
		shard_description:
			"Casting Frosbite on yourself also reduces incoming damage by ${shard_damage_reduction_pct}% for the duration. Allows casting Frostbite while channeling as long as the target is in range.<br><br>Reduces cooldown by ${shard_cooldown_reudction} seconds.",
	...
	Abilities.push({
		...
		reimagined_effects: [
...

			{
				...
				description:
					"Dealing damage to enemy units improves Arcane Aura's mana regeneration by ${blueheart_mastery_mana_regen} for all allies for each damage instance she inflicts. Stacks infinitely and has independent stack durations. Each stack lasts ${blueheart_mastery_duration} seconds.",
			},
		],
		lore: "Cold temperatures promote the essence of magic, causing Rylai's presence to allow spell usage in abundance.",
		notes: ["Blueheart Mastery's mana regen bonus applies after the self factor is taken into account. However, this bonus is multiplied by Frostbite's Igloo Frosting multiplier."],
		ability_specials: [
			{
	...
	Abilities.push({
		...
		name: "Freezing Field",
		description:
			"CHANNELED - Surrounds Crystal Maiden with random icy explosions that slow enemies and deal massive damage. Grants bonus armor while channeling. Lasts ${abilitychanneltime} seconds.",
		reimagined_effects: [
			{
				title: "Arcane Glacier",
				description:
					"For every ${arcane_glacier_interval} seconds that Freezing Field is still channeling, Crystal Maiden's damage resistance increases by ${arcane_glacier_damage_res_per_tick}%. Lasts ${arcane_glacier_linger_duration} seconds after the channeling of Freezing Field ends.",
			},

			{
				title: "Subzero Crystal",
				description:
					"Freezing Field's explosions have a ${subzero_crystal_chance}% chance to leave a Snowstorm Field on the impact location for ${subzero_crystal_duration} seconds, applying the reimaginations of Crystal Nova.",
			},
	...
	Talents.push({
		...
		talents: [
...

			{
				name: "Dense Ice",
				description:
					"Crystal Nova's move and attack speed slow scales with the the enemy's distance to the center of the Snowstorm Field, up to ${max_additional_slow}% additional slow when standing in its center. Every ${units_per_slow} units of distance from the center reduces the slow further.",
...

			{
				name: "Bunker of Ice",
				description:
					"Igloo Frosting now also increases the cast range of the affected ally by ${cast_range_bonus}. Also increases allies' Frost Emanation's search radius by ${frost_emanation_radius_bonus}.",
...

			{
				name: "Intense Cold",
				description:
					"Focused Arcane now multiplies the mana regeneration of Arcane Aura by ${mana_regen_multiplier} and decreases the cooldowns of all affected allies by ${cooldown_reduction}%.",
...

			{
				...
				description:
					"Freezing Field now can be set to auto cast. While auto cast is on, all explosions hit ${explosion_distance} units around Crystal Maiden, moving clockwise, then send a shard in that direction as a linear projectile. Shards deals explosion damage to a single unit.",
