	...
	Abilities.push({
		...
		reimagined_effects: [
...

			{
				...
				description:
					"For every ${momentum_punch_units_travel} units the main projectile traveled until it hit the target or was disjointed, increases the impact's AoE by ${momentum_punch_aoe_increase}.",
			},

			{
				title: "Gomu Gomu No Gatling Gun",
				description:
					"Fires additional ${gatling_gun_count} mini-Storm Hammer projectiles in random positions next to Sven which travel forward in the direction of his target, mini-stunning and dealing ${gatling_gun_damage} damage to enemies it comes in contact with.",
			},
		],
		...
		reimagined_effects: [
			{
				...
				description:
					"Grants Sven and his allies a shield that absorbs up to ${heart_valor_current_hp_shield_pct}% of Sven's current health. Lasts until the shield is depleted or until Warcry's duration ends.",
			},
	...
	Abilities.push({
		...
		reimagined_effects: [
...

			{
				title: "Rogue Knight",
				description:
					"Killing an enemy unit while in God's Strength improves the current God's Strength bonus damage percentage by ${rough_knight_unit_kill_damage_bonus}%. Hero kills grant ${rough_knight_hero_kill_damage_bonus}% instead.",
			},
		],
	...
	Talents.push({
		...
		talents: [
...

			{
				name: "Quakewave",
				description:
					"Overhead Slam now knockbacks enemies hit up and back ${knockback_distance} units over a duration of ${knockback_duration} seconds. Units hit are stunned for a total of ${stun_duration} seconds.",
