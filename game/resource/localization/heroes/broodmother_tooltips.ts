...
export function GenerateLocalizationData(): LocalizationData {
	...
	Abilities.push({
		...
		description:
			"Broodmother injects her young into an enemy unit, dealing damage. The spiderlings will hatch if the target is killed while under this influence, which lasts ${buff_duration} seconds.",
		reimagined_effects: [
			{
				title: "Avenger",
				description:
					"Whenever any Spider unit dies in ${avenger_radius} radius of Broodmother, Broodmother gains a stack of Avenger. Each stack increases her base damage by ${avenger_damage_pct}% for ${avenger_duration} seconds. Has independent stacks.",
			},
		...
		reimagined_effects: [
			{
				...
				description:
					"Poison Sting accumulates stacks on the target each time it is attacked by a Spiderling. When the target reaches ${ticking_poison_stacks} stacks, all stacks are consumed, causing the target to burst and take ${ticking_poison_damage} magical damage.",
			},

			{
				title: "Volatile Spiderlings",
				description:
					"Spiderlings explode on death, dealing ${volatile_spiderling_damage} magical damage in ${volatile_spiderling_radius} radius around them and applying Poison Sting to enemies in the area of effect.",
			},
		],
		notes: ["Volatile Spiderlings does not trigger from expiring."],
		ability_specials: [
			{
		...
		reimagined_effects: [
			{
				title: "Spider Genes",
				description:
					"Each unique Spiderling that attacks the target adds a stack. If the target dies with at least ${spider_genes_stacks} stacks, it will birth a Spiderling instead of a Spiderite.",
			},
		],
		ability_specials: [
			{
	...
	Abilities.push({
		...
		notes: ["Does not trigger when the Spiderite expires."],
		ability_specials: [
			{
		...
		reimagined_effects: [
			{
				title: "Web Manuever",
				description:
					"Increases Spiderlings' and Spiderites' damage reduction by ${web_menuever_dmg_pct}% while they are on a web. In addition, Broodmother and all her spider units gain ${web_menuever_attack_speed} attack speed bonus while on a web.",
			},
		...
		reimagined_effects: [
			{
				title: "Webbed Up",
				description:
					"When Broodmother or a spider unit under her control attack an enemy standing in a web, that enemy gets a stack of Webbed Up. When it reaches ${web_up_stacks_threshold} stacks, stacks are consumed, causing the enemy to become rooted and to have %web_up_miss_chance_pct%%% miss chance for %web_up_duration% second. Broodmother's attacks increase the stacks by %web_up_stacks_hero%. Spiderlings', Spiderites and Spiderking's attacks increase it by ${web_up_stacks_spider} stack instead.",
			},
		...
		talents: [
			{
				name: "Poison Popper",
				description:
					"Spiderlings and Spiderites have a ${pop_chance}% chance to pop when taking damage, triggering their Volatile Poison effect around them. The damage instance must be greater than ${min_damage} after reductions in order to roll the chance to pop.",
...

			{
				name: "Tangled",
				description:
					"Enemy units that move while inside of a web gain a stack for every ${time_threshold} seconds they accumulate. Each stack reduces move speed by ${move_speed_slow_per_stack}, up to a maximum of ${max_stacks} stacks.",
...

			{
				name: "Silken Bind",
				description:
					"Casting Spin Web causes all enemies in Spin Web's radius to become rooted and disarmed for ${bound_duration} seconds. Each enemy can only be afflicted by this effect once every ${trigger_immunity_duration} seconds.",
...

			{
				name: "Weblings",
				description:
					"Spiderlings and Spiderites now apply ${additional_stacks} additional Webbed Up stacks, and gain ${attack_speed_bonus} attack speed when attacking a target with the Webbed Up counter or debuff. The attack speed buff lasts ${attack_speed_duration} seconds.",
