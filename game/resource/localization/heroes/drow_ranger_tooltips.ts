...
export function GenerateLocalizationData(): LocalizationData {
		...
		reimagined_effects: [
			{
				...
				description:
					"Frost Arrows causes the target to lose ${brittle_cold_armor_loss_pct}% of its total armor and ${brittle_cold_aspd_loss%} attack speed for ${brittle_cold_duration} seconds. Stacks indefinitely. Stacks have independent duration.",
			},

			{
				title: "Freezing Offensive",
				description:
					"Each Frost Arrow extends the current slow debuff timer by ${duration} seconds, instead of refreshing it. The timer can only be extended up to a maximum of ${freezing_offensive_max_duration} seconds.",
			},

			{
				title: "Cryo Arrowhead",
				description:
					"While Marksmanship is active, grants a ${cryo_arrowhead_chance}% chance to have Frost Arrows explode on impact, dealing bonus ${cryo_arrowhead_damage} magical damage to all enemies in ${cryo_arrowhead_radius} radius of the target. All enemies hit are afflicted with Frost Arrow's slow debuff for ${cryo_arrowhead_duration} seconds.",
			},
		],
		shard_description:
			"Frost Arrows now apply a Hypothermia stack to enemy heroes, reducing their regeneration by ${shard_health_regen_reduction_stack}% per stack. Increases damage by ${shard_bonus_damage_per_stack} per stack. Lasts ${shard_hypothermia_duration} seconds. If an enemy hero dies with Hypothermia stacks, they burst and deal ${shard_burst_damage} magic damage per stack and ${shard_burst_slow_pct}% slow for ${shard_burst_slow_duration} seconds to enemies within ${shard_burst_damage_radius} radius. Max Stacks: ${shard_max_stacks}.",
	...
	Abilities.push({
		...
		reimagined_effects: [
...

			{
				title: "Wave Chill",
				description:
					"Enemies that are in ${wave_chill_range} range of Drow Ranger are also frozen, becoming stunned during the Gust's knockback plus ${wave_chill_freeze_duration} additional seconds.",
			},

			{
				title: "Tailwind",
				description:
					"Drow can cast Gust on herself to constantly push her on the direction she's currently facing, silencing enemies in ${tailwind_silence_radius} radius around her. Drow Ranger can rotate herself to change the direction the gust pushes her. Lasts ${tailwind_duration} seconds.",
			},
		],
	...
	Abilities.push({
		...
		reimagined_effects: [
...

			{
				...
				description:
					"Can be set to auto cast to increase the max channel time to ${endless_barrage_channel_time} seconds, firing a wave of arrows every ${endless_barrage_delay_between_waves} seconds, but causes each wave to drain ${endless_barrage_mana_per_wave} mana. Lasts until no mana is left to fire an additional wave, the channeling is interrupted, or the duration elapses.",
			},

			{
				title: "Thrilling Hunt",
				description:
					"While Marksmanship is enabled, the first wave's arrows trigger Marksmanship's bonuses. In addition, each enemy hit with a Multishot projectile increases the projectile speed of all further projectiles of this cast by ${thrilling_hunt_projectile_speed}.",
			},
		],
	...
	Abilities.push({
		...
		name: "Marksmanship",
		description:
			"Drow's experience in battle grant her a chance to launch arrows with incredible accuracy and effectiveness. Pierces through the enemy's defenses, ignoring their base armor. Grants Drow and nearby ranged heroes with bonus agility based on Drow's current agility. This ability is disabled if there is an enemy hero within ${disable_range} range.",
		reimagined_effects: [
			{
	...
	Talents.push({
		...
		talents: [
...

			{
				name: "Cryo Surge",
				description:
					"Cryo Arrowhead now applies even when Marksmanship is disabled. Additionally, the chance to proc Cryo Arrowhead increases by ${talent_2_cryo_chance_bonus}% when Marksmanship is active.",
...

			{
				...
				description:
					"During Tailwind, Drow Ranger fires a miniature Gust projectile backwards every ${talent_3_interval} seconds. The projectile has ${talent_3_width} width and goes up to ${talent_3_distance} distance, knocking enemies back and silencing them for ${talent_3_silence_duration} seconds.",
...

			{
				name: "Reflex Shot",
				description:
					"Drow Ranger performs an instant attack on every enemy unit that was hit by ${talent_5_arrows_for_attack} Multishot arrows. This effect can occur multiple times in a single Multishot cast, and counts for enemies that were already hit by an arrow in a wave. The counter resets between casts.",
...

			{
				...
				description:
					"While Pride of the Drow is not active, taking damage applies Pride of the Drow automatically for ${talent_7_pride_duration} seconds. Has a ${talent_7_internal_cd} seconds internal cooldown. Does not trigger Marksmanship's cooldown and can trigger while Marksmanship is on cooldown.",
