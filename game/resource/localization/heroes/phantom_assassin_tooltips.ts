...
export function GenerateLocalizationData(): LocalizationData {
	...
	Abilities.push({
		...
		description:
			"Throws a dagger slowing the enemy unit's movement speed, dealing ${base_damage} + ${attack_factor_tooltip}% of Phantom Assassin's attack damage as physical damage and applying attack effects from items and abilities.",
		reimagined_effects: [
			{
				title: "Fan of Knives",
				description:
					"Throws ${fan_of_knives_add_daggers} additional daggers on random targets in Stifling Dagger's cast range that are not the main target that deal ${fan_of_knives_fixed_damage} physical damage and do not proc on-hit effects. If there is at least one dagger that was not used, one additional dagger is thrown at the main target. Additional daggers still apply the slow debuff.",
			},

			{
				title: "Sharp and Quiet",
				description:
					"Stifling Dagger's chance to proc Coup de Grace increases by ${sharp_and_quite_crit_per_stack}% chance for every ${sharp_and_quite_hp_threshold_per_stack}% health that the enemy unit has.",
			},
	...
	Abilities.push({
		...
		reimagined_effects: [
...

			{
				title: "Escape Plan",
				description:
					"After blinking to an ally, Phantom Assassin gains Blur's invisibility for ${escape_plan_blur_duration} seconds, and gets a ${escape_plan_ms_bonus_pct}% move speed bonus as well for the same duration.",
			},
		],
		...
		reimagined_effects: [
			{
				...
				description:
					"Phantom Assassin instantly attacks any enemy within her attack range that misses her with an auto attack. Each enemy that procs this effect has its own independent cooldown of ${turned_blade_internal_cooldown} seconds.",
			},

			{
				title: "Quick and Quiet",
				description:
					"While Blur is active, grants ${quick_quiet_ms_per_interval}% move speed for every ${quick_quiet_interval} second it is on. Lingers for ${quick_quiet_linger_duration} additional seconds after Blur is dispelled or expires.",
			},
		...
		reimagined_effects: [
			{
				title: "Kiss of Death",
				description:
					"When Phantom Assassin delivers a critical strike from Coup de Grace to a target with less than ${kiss_of_death_health_threshold}% health, there is a ${kiss_of_death_kill_chance}% chance that the critical damage will deal extremely massive damage.",
			},
	...
	Abilities.push({
		...
		name: "Fan of Knives",
		description:
			"Phantom Assassin releases sharp blades around her in a ${radius} AoE, dealing ${pct_health_damage}% of a victim's max health on impact and applying break for ${duration} seconds.",
	});
	...
	Talents.push({
		...
		talents: [
...

			{
				...
				description:
					"Casting Stifling Dagger while under Blur's active effect causes the projectiles to become completely invisible to the enemy. Additionally, the main projectile deals ${talent_2_dagger_bonus_damage} bonus damage, while Fan of Knives projectiles deal ${talent_2_fake_dagger_bonus_damage} bonus damage.",
...

			{
				name: "Strikin' Out",
				description:
					"Each attack done while Phantom Assassin has the attack speed buff from Phantom Strike increases her base damage by ${talent_3_bonus_damage_per_stack_pct}% until the buff ends.",
...

			{
				name: "Secret Blade",
				description:
					"All enemies hit by Nothing Personalle are affected with Stifling Dagger's slow debuff and are ministunned. If they are already afflicted with Stifling Dagger's debuff, then the duration is refreshed and extended by ${talent_4_extend_duration} seconds.",
...

			{
				name: "Mercy Killing",
				description:
					"When Phantom Assassin's attack target has less than ${talent_7_health_threshold}% health, her next attack on it will guarantee a Coup de Grace proc. This effect can trigger only once every ${talent_7_proc_cooldown} seconds.",
...

			{
				name: "Clean Streak",
				description:
					"While Decisive Strike is active, if Phantom Assassin's target is stunned when she begins the attack, the attack speed penalty is reduced by ${talent_8_attack_penalty_reduction_pct}% and Coup De Grace's critical damage increases by ${talent_8_crit_damage_bonus}%.",
