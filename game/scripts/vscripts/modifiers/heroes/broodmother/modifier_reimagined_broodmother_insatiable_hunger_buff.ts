...
export class modifier_reimagined_broodmother_insatiable_hunger_buff extends BaseModifier {
    ...
    modifier_feed_brood_health_buff: string =
        "modifier_reimagined_broodmother_insatiable_hunger_feed_brood_health_buff";
    modified_feed_brood_unit_heal_buff: string =
        "modifier_reimagined_broodmother_insatiable_hunger_feed_brood_unit_heal";
    ...
    OnCreated(): void {
        ...
        // Apply particle
        this.particle_hunger_fx = ParticleManager.CreateParticle(
            this.particle_hunger,
            ParticleAttachment.ABSORIGIN_FOLLOW,
            this.caster
        );
        ParticleManager.SetParticleControlEnt(
            this.particle_hunger_fx,
            0,
            this.parent,
            ParticleAttachment.POINT_FOLLOW,
            AttachLocation.THORAX,
    ...
    ReimaginedTalentFeastFrenzy(event: ModifierAttackEvent) {
        ...
        {
            // Initialize values
            if (!this.talent_8_health_minimum_pct)
                this.talent_8_health_minimum_pct = GetTalentSpecialValueFor(
                    this.caster,
                    BroodmotherTalents.BroodmotherTalent_8,
                    "health_minimum_pct"
                );
            if (!this.talent_8_duration)
                this.talent_8_duration = GetTalentSpecialValueFor(
                    this.caster,
                    BroodmotherTalents.BroodmotherTalent_8,
                    "duration"
                );
            ...
            this.parent.AddNewModifier(this.caster, this.ability, this.modifier_talent_8_buff, {
                duration: this.talent_8_duration,
            });
        }
    }
    ...
    ReimaginedFeedTheBroodApplyDebuff(event: ModifierAttackEvent) {
        ...
        event.target.AddNewModifier(this.parent, this.ability, this.modifier_feed_brood_debuff, {
            duration: this.feed_brood_debuff_duration,
        });
    }

    ReimaginedFeedTheBrood(event: ModifierInstanceEvent) {
        ...
        if (!this.parent.HasModifier(this.modifier_feed_brood_health_buff)) {
            modifier_buff = this.parent.AddNewModifier(
                this.parent,
                this.ability,
                this.modifier_feed_brood_health_buff,
                { duration: this.GetRemainingTime() }
            ) as modifier_reimagined_broodmother_insatiable_hunger_feed_brood_health_buff;
        } else {
            modifier_buff = this.parent.FindModifierByName(
                this.modifier_feed_brood_health_buff
            ) as modifier_reimagined_broodmother_insatiable_hunger_feed_brood_health_buff;
        }
        ...
        let spiderlings = FindUnitsAroundUnit(
            this.parent,
            this.parent,
            this.queen_brood_radius!,
        ...
        for (const spiderling of spiderlings) {
            ...
            if (!spiderling.HasModifier(this.modified_feed_brood_unit_heal_buff)) {
                modifier = spiderling.AddNewModifier(
                    this.parent,
                    this.ability,
                    this.modified_feed_brood_unit_heal_buff,
                    { duration: this.feed_brood_heal_duration }
                );
            } else {
    ...
    GetAuraRadius() {
        return this.queen_brood_radius!;
    }
    GetAuraSearchFlags() {
        return UnitTargetFlags.PLAYER_CONTROLLED;
    }
    GetAuraSearchTeam() {
