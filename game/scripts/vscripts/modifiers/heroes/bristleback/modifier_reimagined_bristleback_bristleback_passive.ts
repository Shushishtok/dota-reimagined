...
export class modifier_reimagined_bristleback_bristleback_passive extends BaseModifier
{
    ...
    ReimaginedBristlefront(event: ModifierAttackEvent): void
    {
        if (HasTalent(this.caster, BristlebackTalents.BristlebackTalent_5))
        {
            // Initialize variables
            if (!this.talent_5_damage_taken_to_threshold_pct) this.talent_5_damage_taken_to_threshold_pct = GetTalentSpecialValueFor(this.caster, BristlebackTalents.BristlebackTalent_5, "damage_taken_to_threshold_pct");
    ...
    ReimaginedTalentBarbedExterior(event: ModifierAttackEvent): void
    {
        if (HasTalent(this.parent, BristlebackTalents.BristlebackTalent_6))
        {
            ...
            if (effectiveDot >= this.effective_dot_back!)
            {
                ...
                ParticleManager.SetParticleControlEnt(this.particle_talent_6_reflect_fx, 1, event.attacker, ParticleAttachment.POINT_FOLLOW, AttachLocation.HITLOC, event.attacker.GetAbsOrigin(), true);
