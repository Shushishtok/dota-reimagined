...
import {
    ...
    ConvertRadiansToEffectiveDotRange,
    GetAttackDotProduct,
...
export class modifier_reimagined_bristleback_bristleback_passive extends BaseModifier {
    ...
    GetAbilitySpecialValues(): void {
        ...
        this.moving_fortress_damage_reduction_bonus = this.ability.GetSpecialValueFor(
            "moving_fortress_damage_reduction_bonus"
        );
    ...
    GetModifierIncomingDamage_Percentage(event: ModifierAttackEvent): number {
        ...
        if (HasBit(event.damage_flags, DamageFlag.HPLOSS) || HasBit(event.damage_flags, DamageFlag.REFLECTION))
            return 0;
        ...
        if (this.effective_dot_back && dot_product >= this.effective_dot_back) {
            ...
            this.particle_back_fx = ParticleManager.CreateParticle(
                this.particle_back,
            ...
            if (event.damage >= 200) {
                this.particle_back_large_fx = ParticleManager.CreateParticle(
                    this.particle_back_large,
                    ParticleAttachment.ABSORIGIN_FOLLOW,
                    this.parent
                );
    ...
    IncreaseQuillTriggerCounter(increase_by: number) {
        ...
        const ability_quill_spray_handle = this.parent.FindAbilityByName(
            this.ability_quill_spray
        ) as reimagined_bristleback_quill_spray;
    ...
    ReimaginedBristlefront(event: ModifierInstanceEvent): void {
        if (HasTalent(this.caster, BristlebackTalents.BristlebackTalent_5)) {
            ...
            if (!this.talent_5_damage_taken_to_threshold_pct)
                this.talent_5_damage_taken_to_threshold_pct = GetTalentSpecialValueFor(
                    this.caster,
                    BristlebackTalents.BristlebackTalent_5,
                    "damage_taken_to_threshold_pct"
                );
    ...
    ReimaginedTalentBarbedExterior(event: ModifierAttackEvent): void {
        if (HasTalent(this.parent, BristlebackTalents.BristlebackTalent_6)) {
            ...
            if (!this.talent_6_damage_reflection_pct)
                this.talent_6_damage_reflection_pct = GetTalentSpecialValueFor(
                    this.parent,
                    BristlebackTalents.BristlebackTalent_6,
            ...
            if (effectiveDot >= this.effective_dot_back!) {
                ...
                // Play particle
                this.particle_talent_6_reflect_fx = ParticleManager.CreateParticle(
                    this.particle_talent_6_reflect,
