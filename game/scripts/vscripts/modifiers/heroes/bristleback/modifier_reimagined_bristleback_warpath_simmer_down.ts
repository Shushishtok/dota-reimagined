...
export class modifier_reimagined_bristleback_warpath_simmer_down extends BaseModifier {
	...
	ReimaginedTalentPersistentBrawler(): number {
		if (HasTalent(this.caster, BristlebackTalents.BristlebackTalent_8)) {
			...
			if (!this.talent_8_cooldown_reduction_per_stack)
				this.talent_8_cooldown_reduction_per_stack = GetTalentSpecialValueFor(this.caster, BristlebackTalents.BristlebackTalent_8, "cooldown_reduction_per_stack");
