...
export class modifier_reimagined_phantom_assassin_coup_de_grace_decisive_strike extends BaseModifier {
	...
	DeclareFunctions(): ModifierFunction[] {
		return [ModifierFunction.ON_ATTACK_START, ModifierFunction.ATTACKSPEED_BONUS_CONSTANT, ModifierFunction.TOOLTIP];
	}
	...
	ReimaginedTalentCleanStreak(decisive_strike_as_reduction: number): number {
		if (HasTalent(this.parent, PhantomAssassinTalents.PhantomAssassinTalent_8)) {
			...
			if (!this.talent_8_attack_penalty_reduction_pct)
				this.talent_8_attack_penalty_reduction_pct = GetTalentSpecialValueFor(this.caster, PhantomAssassinTalents.PhantomAssassinTalent_8, "talent_8_attack_penalty_reduction_pct");
