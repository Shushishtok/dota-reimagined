...
export class modifier_reimagined_phantom_assassin_stifling_dagger_caster extends BaseModifier {
	...
	DeclareFunctions(): ModifierFunction[] {
		return [ModifierFunction.BASEDAMAGEOUTGOING_PERCENTAGE, ModifierFunction.PREATTACK_BONUS_DAMAGE];
	}
	...
	ReimaginedTalentFromTheShadowsApplyDamage(): number {
		if (this.talent_2_triggered) {
			if (!this.talent_2_dagger_bonus_damage)
				this.talent_2_dagger_bonus_damage = GetTalentSpecialValueFor(this.caster, PhantomAssassinTalents.PhantomAssassinTalent_2, "talent_2_dagger_bonus_damage");
