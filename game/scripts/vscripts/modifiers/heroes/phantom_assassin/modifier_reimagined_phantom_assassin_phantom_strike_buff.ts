...
export class modifier_reimagined_phantom_assassin_phantom_strike_buff extends BaseModifier {
	...
	ReimaginedTalentStrikinOutDamage(): number {
		if (HasTalent(this.caster, PhantomAssassinTalents.PhantomAssassinTalent_3)) {
			if (!this.talent_3_bonus_damage_per_stack_pct)
				this.talent_3_bonus_damage_per_stack_pct = GetTalentSpecialValueFor(this.caster, PhantomAssassinTalents.PhantomAssassinTalent_3, "talent_3_bonus_damage_per_stack_pct");
