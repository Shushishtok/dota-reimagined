...
export class modifier_reimagined_phantom_assassin_blur_active extends BaseModifier {
	...
	ReimaginedQuickAndQuiet(destroy: boolean): void {
		...
		if (!destroy) {
			if (!this.parent.HasModifier(modifier_reimagined_phantom_assassin_blur_quick_and_quiet.name)) {
				this.parent.AddNewModifier(this.parent, this.ability, modifier_reimagined_phantom_assassin_blur_quick_and_quiet.name, {
					duration: this.GetDuration() + this.quick_quiet_linger_duration!,
				});
			} else {
	...
	CheckState(): Partial<Record<ModifierState, boolean>> {
		return { [ModifierState.INVISIBLE]: true, [ModifierState.TRUESIGHT_IMMUNE]: true };
	}
