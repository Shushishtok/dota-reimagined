...
interface ReflectedAbility
{
    spell_shield_reflect: boolean;
}

// Type guard - to check if an ability has ReflectAbility stuff
function isReflectedAbility(ability: CDOTABaseAbility): ability is CDOTABaseAbility & ReflectedAbility 
{
    return (ability as any).spell_shield_reflect !== undefined;
}

// Add the Reflect Ability tag to an existing ability
function makeReflectAbility(ability: CDOTABaseAbility): CDOTABaseAbility & ReflectedAbility 
{
...
export class modifier_reimagined_antimage_counterspell_active extends BaseModifier
{
    ...
    GetReflectSpell(event: ModifierAbilityEvent): 0 | 1
    {
        ...
        // Do not reflect spells inside the exception table
        if (exceptionAbilities.includes(event.ability.GetAbilityName())) return 0;

        if (isReflectedAbility(event.ability)) return 0;
        
        // If the parent already knows the ability, reference it
