...
export class modifier_reimagined_night_stalker_dark_ascension_wings_out extends BaseModifier {
	...
	DeclareFunctions(): ModifierFunction[] {
		return [ModifierFunction.PREATTACK_BONUS_DAMAGE, ModifierFunction.TRANSLATE_ACTIVITY_MODIFIERS, ModifierFunction.MOVESPEED_BONUS_PERCENTAGE];
	}
	...
	ShouldModifierBeActive(): boolean {
		...
		if (
			...
			this.parent.GetModifierStackCount(modifier_reimagined_night_stalker_hunter_in_the_night_dead_of_night.name, this.parent) >= this.wings_out_stack_threshold
		) {
			return true;
		}
