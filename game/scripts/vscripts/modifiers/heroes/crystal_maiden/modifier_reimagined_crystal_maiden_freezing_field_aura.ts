...
export class modifier_reimagined_crystal_maiden_freezing_field_aura extends BaseModifier {
	...
	GetModifierAura() {
		return "modifier_reimagined_crystal_maiden_freezing_field_slow";
	}
	...
	ReimaginedTalentWhirlwindOfFrost(): boolean {
		if (HasTalent(this.caster, CrystalMaidenTalents.CrystalMaidenTalent_8)) {
			...
			if (
				this.caster.IsStunned() ||
				this.caster.IsHexed() ||
				this.caster.IsSilenced() ||
				this.caster.IsOutOfGame() ||
				this.caster.IsNightmared() ||
				this.caster.IsFrozen() ||
				this.caster.IsCommandRestricted() ||
				!this.caster.IsAlive() ||
				this.caster.GetForceAttackTarget() != undefined
			) {
				// Stop casting this
