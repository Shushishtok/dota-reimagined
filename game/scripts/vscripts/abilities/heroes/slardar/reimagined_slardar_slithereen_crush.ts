...
export class reimagined_slardar_slithereen_crush extends BaseAbility
{
    ...
    ReimaginedSplashAttack(splash_attack: boolean, force_river_values: boolean): number
    {
        if (IsInRiver(this.caster))
        {
            if (splash_attack)
            {
                ...
                {
                    const position = GenerateRandomPositionAroundPositionAngled(this.caster.GetAbsOrigin(), this.caster.GetForwardVector(), 300, this.splash_attack_puddle_search_radius!, index * 90, index + 1 * 90);
    ...
    ReimaginedBrineBreeze(casting_unit: CDOTA_BaseNPC)
    {
        ...
        const enemies_outer_ring = FindUnitsInRadiusOuterRing(this.caster,
