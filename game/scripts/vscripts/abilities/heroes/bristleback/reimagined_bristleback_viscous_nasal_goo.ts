...
export class reimagined_bristleback_viscous_nasal_goo extends BaseAbility {
	...
	cast_responses = [
		"bristleback_bristle_nasal_goo_01",
		"bristleback_bristle_nasal_goo_02",
		"bristleback_bristle_nasal_goo_03",
		"bristleback_bristle_nasal_goo_04",
		"bristleback_bristle_nasal_goo_05",
		"bristleback_bristle_nasal_goo_06",
		"bristleback_bristle_nasal_goo_07",
	];
	...
	ReimaginedTalentMucusGoo(target: CDOTA_BaseNPC) {
		if (util.HasTalent(this.caster, BristlebackTalents.BristlebackTalent_1)) {
			...
			} // Add the modifier to the target and add stacks to it
			else {
	...
	ReimaginedTalentSnotArtillery(): number {
		...
		if (util.HasTalent(this.caster, BristlebackTalents.BristlebackTalent_2)) {
			// Initialize variables
			if (!this.talent_2_cast_range_bonus_per_stack)
				this.talent_2_cast_range_bonus_per_stack = util.GetTalentSpecialValueFor(this.caster, BristlebackTalents.BristlebackTalent_2, "cast_range_bonus_per_stack");
