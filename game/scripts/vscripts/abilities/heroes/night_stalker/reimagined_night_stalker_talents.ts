...
export class modifier_reimagined_night_stalker_talent_6 extends BaseTalentModifier {
    ...
    OnCreated(): void {
        if (!IsServer()) return;
        this.proximity_distance = GetTalentSpecialValueFor(
            this.caster,
            NightStalkerTalents.NightStalkerTalents_6,
            "proximity_distance"
        );
...
export class modifier_reimagined_night_stalker_talent_8 extends BaseTalentModifier {
    ...
    OnCreated(): void {
        this.talent_stacks_threshold = GetTalentSpecialValueFor(
            this.parent,
            NightStalkerTalents.NightStalkerTalents_8,
            "talent_stacks_threshold"
        );
        this.damage_reduction_pct = GetTalentSpecialValueFor(
            this.parent,
            NightStalkerTalents.NightStalkerTalents_8,
            "damage_reduction_pct"
        );
        this.status_resist_pct = GetTalentSpecialValueFor(
            this.parent,
            NightStalkerTalents.NightStalkerTalents_8,
            "status_resist_pct"
        );
        this.damage_amp_pct = GetTalentSpecialValueFor(
            this.parent,
            NightStalkerTalents.NightStalkerTalents_8,
            "damage_amp_pct"
        );
    ...
    ModifierActive(): boolean {
        ...
        if (
            this.parent.GetModifierStackCount(this.modifier_dead_of_night, this.parent) >= this.talent_stacks_threshold!
        ) {
            return true;
        } else {
            return false;
