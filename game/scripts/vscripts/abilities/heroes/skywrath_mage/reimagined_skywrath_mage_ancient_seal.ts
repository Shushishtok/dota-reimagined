...
export class reimagined_skywrath_mage_ancient_seal extends BaseAbility {
	...
	OnSpellStart(): void {
		...
		if (this.caster.HasScepter()) {
			...
			const enemy_heroes = util.FindUnitsAroundUnit(this.caster, target, this.scepter_radius, UnitTargetTeam.ENEMY, UnitTargetType.HERO, UnitTargetFlags.FOW_VISIBLE + UnitTargetFlags.NO_INVIS);
			...
			if (!target_found) {
				const enemy_creeps = util.FindUnitsAroundUnit(
					this.caster,
					target,
					this.scepter_radius,
	...
	ReimaginedSealofScreeauk(target: CDOTA_BaseNPC): boolean {
		...
		if (target.GetTeamNumber() == this.caster.GetTeamNumber()) {
			...
			if (this.caster.HasScepter()) {
				let target_found = false;
				const friendly_heroes = util.FindUnitsAroundUnit(this.caster, target, this.scepter_radius!, UnitTargetTeam.FRIENDLY, UnitTargetType.HERO, UnitTargetFlags.NONE);
				...
				// If we didn't find any hero, then cast it on a nearby creep instead
				if (!target_found) {
					const friendly_creeps = util.FindUnitsAroundUnit(this.caster, target, this.scepter_radius!, UnitTargetTeam.FRIENDLY, UnitTargetType.BASIC, UnitTargetFlags.NONE);
