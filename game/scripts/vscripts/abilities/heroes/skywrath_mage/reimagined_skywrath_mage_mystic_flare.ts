...
export class reimagined_skywrath_mage_mystic_flare extends BaseAbility {
	...
	cast_responses: string[] = [
		"skywrath_mage_drag_mystic_flare_01",
		"skywrath_mage_drag_mystic_flare_02",
		"skywrath_mage_drag_mystic_flare_03",
		"skywrath_mage_drag_mystic_flare_04",
		"skywrath_mage_drag_mystic_flare_05",
	];
	...
	OnSpellStart(): void {
		...
		if (this.caster.HasScepter()) {
			const enemy_heroes = FindUnitsInRadius(
			...
			const enemy_creeps = FindUnitsInRadius(
				this.caster.GetTeamNumber(),
				target_position,
				undefined,
				this.scepter_radius,
		...
		if (this.mystic_flare_map.has(mystic_flare_ID)) {
			...
			Timers.CreateTimer(properties.damage_interval, () => {
				...
				if (enemies.length > 0) {
					...
					// Minimum distance to search for an hero
					if (util.CalculateDistanceBetweenPoints(enemy_pos, properties.position) > 125) {
						// Get direction
