...
export class reimagined_skywrath_mage_concussive_shot extends BaseAbility {
	...
	OnSpellStart(): void {
		...
		if (this.caster.HasScepter()) {
			...
			let scepter_target;
			let scepter_enemies = FindUnitsInRadius(
				this.caster.GetTeamNumber(),
				this.caster.GetAbsOrigin(),
				undefined,
				this.launch_radius,
	...
	LaunchConcussiveShot(
		source: CDOTA_BaseNPC,
		target: CDOTA_BaseNPC,
		conjured_relay_info: { remaining_bounces: number; main_target: CDOTA_BaseNPC; enemies_hit: Set<CDOTA_BaseNPC> },
		primary_projectile: boolean
	) {
	...
	OnProjectileHitHandle(target: CDOTA_BaseNPC, location: Vector, projectile_handle: ProjectileID) {
		...
		const enemies = util.FindUnitsAroundUnit(this.caster, target, radius, UnitTargetTeam.ENEMY, UnitTargetType.HERO + UnitTargetType.BASIC, UnitTargetFlags.NONE);
	...
	ReimaginedGhastlyEerie(target: CDOTA_BaseNPC): { radius: number; duration: number } {
		let effect_properties: { radius: number; duration: number } = { radius: this.slow_radius!, duration: this.slow_duration! };
