...
class CustomChat {
    ...
    GetChatContainerPanel() {
        this.chatContainerPanel =
            this.hudPanel!.FindChildTraverse("ChatLinesPanel")!;
    }
    ...
    OnChatMessage(event: {
        ...
        const message = $.CreatePanelWithProperties(
            "Label",
            this.chatContainerPanel!,
            "",
            {
        ...
        if (event.playerID > -1) {
            ...
            const playerColor =
                "#" + this.intToARGB(Players.GetPlayerColor(event.playerID));

            text += event.isTeam
                ? `[${$.Localize("#DOTA_ChatCommand_GameAllies_Name")}] `
                : this.NON_BREAKING_SPACE;
            text += `<font color='${playerColor}'>${playerInfo.player_name}</font>: `;

            $.CreatePanelWithProperties("Panel", message, "", {
                class: "HeroBadge",
                selectionpos: "auto",
            });

            const heroIcon = $.CreatePanelWithProperties("Image", message, "", {
                class: "HeroIcon",
                selectionpos: "auto",
            });
            heroIcon.SetImage(
        ...
        text += event.textData.replace(/%%\d*(.+?)%%/g, (_, token) =>
            $.Localize(token)
        );
        if (event.ability_name != undefined) {
            text = GameUI.ReplaceDOTAAbilitySpecialValues(
                event.ability_name,
                text
            )!;
        }
