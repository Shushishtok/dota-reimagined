...
class RmgTalentWindow
{
    ...
    AddRmgHudTalentButton()
    {
        ...
        const abilityBar = this.GetHudRoot().FindChildTraverse("HUDElements")!.FindChildTraverse("lower_hud")!.FindChildTraverse("center_with_stats")!.FindChildTraverse("center_block")!.FindChildTraverse("AbilitiesAndStatBranch")!.FindChildTraverse("StatBranch")!.GetParent()!;
    ...
    RemoveDotaTalentTree()
    {
        ...
        const talentTree = this.GetHudRoot().FindChildTraverse("HUDElements")!.FindChildTraverse("lower_hud")!.FindChildTraverse("center_with_stats")!.FindChildTraverse("center_block")!.FindChildTraverse("AbilitiesAndStatBranch")!.FindChildTraverse("StatBranch")!;
        ...
        const levelUpButton = this.GetHudRoot().FindChildTraverse("HUDElements")!.FindChildTraverse("lower_hud")!.FindChildTraverse("center_with_stats")!.FindChildTraverse("center_block")!.FindChildTraverse("level_stats_frame")!;
    ...
    {        
        if (this.currentlySelectedUnitID)
        {
            if (Entities.IsValidEntity(this.currentlySelectedUnitID) && Entities.IsRealHero(this.currentlySelectedUnitID) && Entities.IsControllableByPlayer(this.currentlySelectedUnitID, Players.GetLocalPlayer()))
            {                
                $.Schedule(0, () =>
                {                    
                    if (this.CanHeroUpgradeAnyTalent())                    
    ...
    {        
        ...
        const formattedTooltip = this.FormatTooltip(title, description, lore);

        $.DispatchEvent("DOTAShowTextTooltip", button, formattedTooltip);
    }
    ...
    FormatTooltip(title: string, description: string, lore: string): string
    {        
        if (title != "")
        {
            // Bold the title
